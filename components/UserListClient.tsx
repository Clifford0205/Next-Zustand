...
export function UserListClient() {
  const { data: users, isLoading, error, isStale, isFetching } = useQuery({
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div className="mt-6 p-4 bg-muted rounded-lg text-sm text-muted-foreground">
          <p className="font-medium mb-2">💡 SSR + React Query 混合模式功能：</p>
          <ul className="space-y-1">
            <li>• ⚡ 伺服器端預取 - 更快的首次載入</li>
            <li>• 🔄 客戶端 hydration - 無縫接管</li>
            <li>• 📱 背景重新整理 - 資料保持最新</li>
            <li>• 🚀 最佳化快取策略 - 效能與體驗兼顧</li>
            <li>• 🎯 狀態：{isStale ? '資料過時' : '資料新鮮'} | {isFetching ? '正在更新' : '閒置中'}</li>
          </ul>
        </div>
      </CardContent>
    </Card>
